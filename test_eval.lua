...
cmd:option('-crop_size', 224, 'size of crop augment')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    if opt.image_augment == 1 then
      ...
      local crop_size = 200
      local numcrops = opt.iterx*opt.itery
      for ix =1, math.abs(opt.iterx) do
        local tmpy = opt.itery/math.abs(opt.itery)
        for iy =1, math.abs(opt.itery) do
          add_crop(sum_array, ori_images,1, (1-opt.ori_weight)/numcrops, opt.crop_size,-1,false, nil, nil, false,0, tmpx, tmpy)
          tmpy = tmpy + torch.floor((256-crop_size)/opt.itery)
        end
        tmpx = tmpx + torch.floor((256-crop_size)/opt.iterx)
      end
